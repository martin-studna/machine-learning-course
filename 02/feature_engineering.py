...
parser.add_argument("--dataset", default="wine", type=str,
...
def main(args: argparse.Namespace) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Find indices of columns that contains only integers or that have at least one float value.
    int_col_indices = list(np.where(np.mod(dataset.data[0, :], 1) == 0)[0])
    float_col_indices = list(np.where(np.mod(dataset.data[0, :], 1) != 0)[0])
    ...
    polynomial_transformer = sklearn.preprocessing.PolynomialFeatures(
...
if __name__ == "__main__":
    ...
    for dataset in [train_data, test_data]:
        for line in range(min(dataset.shape[0], 5)):
            print(" ".join("{:.4g}".format(dataset[line, column]) for column in range(
                min(dataset.shape[1], 140))))

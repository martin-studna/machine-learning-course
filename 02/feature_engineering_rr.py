...
def main(args: argparse.Namespace) -> tuple[np.ndarray, np.ndarray]:
    ...
    integer_col_indices = np.where((np.mod(dataset.data, 1) != 0).sum(axis = 0) == 0)[0]
    ...
    dtype_handler = sklearn.compose.ColumnTransformer(
        [("encoder", onehot, integer_col_indices),
        ("scaler", scaler, float_col_indices)])
